...
> data files saved in one directory, each of which is formatted like this, all
> the files end with `.txt` extension:
>
> ~~~
> 2013-11-05,deer,5
...
> {: .source}
>
> She wants to extract a list of dates and the number of that species seen on each date.
> For example using the data shown above, would output the following output:
>
> ~~~
> 2013-11-05,22
> 2013-11-06,19
...
> {: .source}
>
> To get the answer for spiecies 'deer', rearrange these commands and pipes in
> the right order:
>
> ~~~
> cut -d : -f 2
...
> >
> > ```
> > grep -w "deer" -r *.txt | cut -d : -f 2 | cut -d , -f 1,3
> > ```
> > {: .language-bash}
> >
> > There is also a shorter answer if using the `grep` option `-h`:
> >
> > ```
> > grep -w -h "deer" -r *.txt | cut -d , -f 1,3
> > ```
> > {: .language-bash}
> > This tells `grep` not to print filename header for found matches in case we
> > use `-r` option to search across many files, so we can ignore the first `cut` command.
> {: .solution}
{: .challenge}
